...
def cvar_gaussian(r, level=5, modified=False):
    """
    Computes the Conditional VaR of Series or DataFrame using Gaussian and Cornish-Fisher
    """
...
def gbm_fast(n_years: int=10, 
             ...
             s_0: float=100.0,
             rng=_rng,
             dtype=np.float32) -> pd.DataFrame | np.ndarray:
    ...
    inc[0] = 1
    if prices:
        ret_val = s_0 * np.cumprod(inc, axis=0, dtype=dtype)
